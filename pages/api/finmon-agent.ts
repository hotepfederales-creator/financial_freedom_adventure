...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { type, data, history, message, userLevel, finMonState, memoryContext } = req.body;
  ...
  try {
    ...
    if (type === 'chat_professor') {
      // Inject memory context into the last user message or as a system instruction tweak
      // Here we append it to the user message for better attention
    ...
    if (type === 'chat_finmon') {
      ...
      const chat = ai.chats.create({
        ...
        history: history || [],
        config: {
          systemInstruction: getFinMonPersona(finMonState)
        }
      });
